...
vi getheight(vi p){
	vi s,res;
	for(auto v:p){
		while(si(s)&&s.back()<v)s.pop_back();
		res.pb(si(s));
		s.pb(v);
	}
	return res;
...
signed main(){
	...
	using T=tuple<vc<mint>,vc<mint>,vc<mint>>;
	//xhout,yvout,yhout
	
	//vout
	auto rectangle_sub=[&](const vc<mint>&vin,const vc<mint>&hin){
		int w=si(vin)-1,h=si(hin)-1;
		assert(w>=0);
		assert(h>=0);
		vc<mint> vout(w+1);
		{
			vc<mint> a(w+1);
			rep(i,w+1)a[i]=binom(i,h);
			auto b=multiply(vin,a);
			rep(i,w+1)vout[i]+=b[i];
		}
		{
			vc<mint> tmp=hin;
			rep(i,h+1)tmp[i]*=finv[h-i];
			vc<mint> a(h+w+1);
			rep(i,h+w+1)a[i]=fact[i];
			auto b=multiply(tmp,a);
			rep(i,w+1)vout[i]+=b[h+i]*finv[i];
		}
		return vout;
	};
	
	//v,h
	auto rectangle=[&](const vc<mint>&vin,const vc<mint>&hin){
		return mp(rectangle_sub(vin,hin),rectangle_sub(hin,vin));
	};
	
	auto dfs=[&](auto self,int lpos,int rpos,int xoff,int yoff,
		vc<mint> xvin,vc<mint> xhin,int xhouts,
		vc<mint> yhin,int yhouts)->T{
		vc<mint> xhout(xhouts),yvout(rpos-lpos+1),yhout(yhouts);
		if(lpos==rpos){
			mint cur=xvin[0];
			rep(i,max(si(xhin),si(xhout))){
				if(i<si(xhin))cur+=xhin[i];
				if(i<si(xhout))xhout[i]=cur;
			}
			per(i,max(si(yhin),si(yhout))){
				if(i<si(yhin))cur+=yhin[i];
				if(i<si(yhout))yhout[i]=cur;
			}
			yvout[0]=cur;
		}else{
			int xdif=*min_element(x.bg+lpos,x.bg+rpos)-xoff;
			if(si(xhin)<xdif)xhin.resize(xdif);
			xoff+=xdif;
			if(xdif>0){
				auto [v,h]=rectangle(xvin,vc<mint>(xhin.bg,xhin.bg+xdif));
				xvin.swap(v);
				rep(i,min(xdif,xhouts))xhout[i]=h[i];
			}
			int ydif=*min_element(y.bg+lpos,y.bg+rpos)-yoff;
			if(si(yhin)<ydif)yhin.resize(ydif);
			yoff+=ydif;
			
			int mpos=(lpos+rpos)/2;
			
			auto [xhl,yvl,yhl]=self(self,lpos,mpos,xoff,yoff,
				vc<mint>(xvin.bg,xvin.bg+(mpos-lpos)+1),
				vc<mint>(xhin.bg+xdif,xhin.ed),
				x[mpos]-xoff,
				vc<mint>(yhin.bg+ydif,yhin.ed),
				y[mpos]-yoff+1);
			xhl.pb(yhl.back());
			yhl.pop_back();
			
			auto [xhr,yvr,yhr]=self(self,mpos+1,rpos,xoff,yoff,
				vc<mint>(xvin.bg+(mpos-lpos)+1,xvin.ed),
				xhl,
				max<int>(xhouts-xdif,0),
				yhl,
				max<int>(yhouts-ydif,0));
			rep(i,si(xhr))xhout[xdif+i]=xhr[i];
			rep(i,si(yhr))yhout[ydif+i]=yhr[i];
			
			yvl.insert(yvl.ed,all(yvr));
			if(ydif>0){
				vc<mint> tmp(ydif);
				rep(i,ydif)tmp[i]=yhin[ydif-1-i];
				auto [v,h]=rectangle(yvl,tmp);
				yvl.swap(v);
				rep(i,min(ydif,yhouts))yhout[i]=h[ydif-1-i];
			}
			assert(si(yvout)==si(yvl));
			yvout.swap(yvl);
		}
		return T(xhout,yvout,yhout);
	};
	
	vc<mint> tmp(n+1,0);
	tmp[0]=1;
	auto ans=dfs(dfs,0,n,0,0,tmp,{},0,{},0);
	print(get<1>(ans)[n]);
}
