...
# for g in grid:
#     print("grid:", g, file = open("log.txt", mode = "a", encoding = "utf-8"))
check = 0
for i in range(N*N):
    seen.append((r, c))
    diceRoll(dir)
    ...
    vcount[dice[1]][pv] += cv
    # print("vc:",dice[roll[dir][1]], file = open("log.txt", mode = "a", encoding = "utf-8"))
    # print("pc:", pv, "cv:", cv, file = open("log.txt", mode = "a", encoding = "utf-8"))
    # if dice[roll[dir][1]] == 1 and pv == 7:
    #     check += cv
    # if dice[roll[dir][1]] == 5 and pv == 4:
    #     print("r:", r, "c:", c, "pv:", pv, "cv:", cv, file = open("log.txt", mode = "a", encoding = "utf-8"))
    # print("r:", r, "c:", c, "bottom:", dice[1], "pv:", pv, "cv:", cv, file = open("log.txt", mode = "a", encoding = "utf-8"))
...
# print("seen:", seen, file = open("log.txt", mode = "a", encoding = "utf-8"))

# print("vc:",vcount, file = open("log.txt", mode = "a", encoding = "utf-8"))
# print("check:",check, file = open("log.txt", mode = "a", encoding = "utf-8"))

for i in range(6):
    keys = list(sorted(vcount[i].items(), key = lambda x:x[1]))
    # print("========================================", file = open("log.txt", mode = "a", encoding = "utf-8"))
    # print("i:",i,"keys:",keys, file = open("log.txt", mode = "a", encoding = "utf-8"))
    dice[i] = keys[-1][0]
    check += keys[-1][1]
