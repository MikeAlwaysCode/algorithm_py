...
class Solution:
    def countBalls(self, lowLimit: int, highLimit: int) -> int:
        # cnt = Counter(list(sum(list(map(int, list(str(num))))) for num in range(lowLimit, highLimit + 1)))
        cnt = Counter(sum(map(int, str(num))) for num in range(lowLimit, highLimit + 1))
