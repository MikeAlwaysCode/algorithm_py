...
class Solution:
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        n = len(tasks)
        idx = sorted(range(n), key = lambda x: tasks[x][0])
        ans = []
        h = []
        i = curr = 0
        while h or i < n:
            if not h and i < n and curr < tasks[idx[i]][0]: curr = tasks[idx[i]][0]
            while i < n and tasks[idx[i]][0] <= curr:
                heappush(h, (tasks[idx[i]][1], idx[i]))
                i += 1
