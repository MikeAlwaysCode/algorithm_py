/*
 * @lc app=leetcode.cn id=2546 lang=golang
 *
 * [2546] 执行逐位运算使字符串相等
 *
...
func makeStringsEqual(s string, target string) bool {
	if s != target && (strings.Count(s, "1") == 0 || strings.Count(target, "1") == 0) {
		return false
	}
	// return s == target || (strings.Contains(s, "1") == strings.Contains(target, "1"))
}
